...
export default function MyLearningsRedirect() {
  ...
  useEffect(() => {
    if (!loading) {
      if (isAuthenticated) {
        router.replace('/user-dashboard/my-notes');
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-20 min-h-[300px]" style={{ background: 'var(--color-bg)' }}>
      ...
      <span className="text-xs font-mono text-[var(--color-text-secondary)] mt-3">Redirecting to notes...</span>
    </div>
  );
}
